                    ...
                    for arg in fdesc:
                        ...
                        if arg_name != "" and arg_type != "":
                            ...
                            if res is not None:
                                ...
                                if "in" in arg_type and "out" in arg_type:
                                    fid = res[0]
                                    aid = res[1]
                                    update = "update FunctionsArgs set Input = 1 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    update = "update FunctionsArgs set Output = 1 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    count += 1

                                # Just output
                                elif "out" in arg_type:
                                    fid = res[0]
                                    aid = res[1]
                                    update = "update FunctionsArgs set Input = 0 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    update = "update FunctionsArgs set Output = 1 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    count += 1
                                ...
                                # Just input
                                    ...
                                    update = "update FunctionsArgs set Input = 1 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    update = "update FunctionsArgs set Output = 0 where Id = %d and FuncId = %d;" % (aid,fid)
                                    c.execute(update)
                                    count += 1
